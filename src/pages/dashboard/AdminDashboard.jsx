...
import { HeartPulse, LogOut, Plus, Users, Stethoscope, Building2, LayoutDashboard, ClipboardList } from "lucide-react";
        ...
        <div className="mt-8 flex gap-6">
          ...
          <aside className="sticky top-8 hidden h-fit w-56 shrink-0 flex-col rounded-2xl bg-white p-4 shadow-md shadow-slate-300/40 lg:flex">
            <nav className="space-y-1">
              ...
              <SidebarLink icon={Stethoscope} label="Doctors" />
              <SidebarLink icon={Users} label="Patients" />
            </nav>

            <div className="mt-6 space-y-2 border-t border-slate-100 pt-4">
              <button
                className="flex w-full items-center gap-3 rounded-xl bg-emerald-700 px-3 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-800"
                type="button"
                onClick={() => setIsAddDoctorOpen(true)}
              >
                <Plus className="h-4 w-4 shrink-0" /> Add Doctor
              </button>
              <button
                className="flex w-full items-center gap-3 rounded-xl bg-slate-800 px-3 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-900"
                type="button"
                onClick={() => setIsAddDepartmentOpen(true)}
              >
                <Building2 className="h-4 w-4 shrink-0" /> Add Department
              </button>
            </div>
          </aside>
          ...
          <div className="flex-1 space-y-6">
            ...
            <section className="flex gap-4" aria-label="Daily summary">
              <SummaryCard icon={Users} label="Patients waiting" value={totalPatients} detail="Across all departments" />
              <SummaryCard icon={Stethoscope} label="Doctors on duty" value={`${doctors.filter((d) => d.status === "Available").length} / ${doctors.length}`} detail="Assigned to today" />
            </section>
            ...
            <section className="rounded-2xl bg-white p-6 shadow-md shadow-slate-300/40">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <select className="select select-bordered w-full border-slate-300 bg-white sm:w-56" value={department} onChange={(event) => setDepartment(event.target.value)} aria-label="Filter queues by department">
